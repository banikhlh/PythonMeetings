...
from defs import create_meeting, get_data_from_db, login, logout, open_connect, register
...
@app.post("/login")
async def web_login(
    request: Request, username: str = Form(None), password: str = Form(None)
):
    status_code = ""
    text, status_code = login(username, password)
    if text != "Login successful":
        context = {"request": request, "data_num": status_code, "data": text, "src": ""}
        return templates.TemplateResponse("template_error.html", context)
    elif text == "Login successful":
        context = {"request": request, "data": text}
        html_content = templates.TemplateResponse("template.html", context).body.decode(
            "utf-8"
        )
        response = HTMLResponse(content=html_content)
        response.set_cookie(
            key="session_token", value=status_code, secure=True, httponly=True
        )
        return response


@app.post("/logout")
async def web_logout(request: Request, session_token=Cookie(None)):
    num = ""
    txt, num = logout(session_token)
    if num != "":
        context = {"request": request, "data_num": num, "data": txt, "src": ""}
        return templates.TemplateResponse("template_error.html", context)
    elif num == "":
        context = {"request": request, "data": txt}
...
async def web_registration(
    ...
    if txt != "Registration successful":
        ...
        response = HTMLResponse(content=html_content)
        response.set_cookie(key="session_token", value=num, secure=True, httponly=True)
        return response


@app.post("/create_meeting", response_class=JSONResponse)
async def web_create_meet(
    request: Request,
    username: str = Form(None),
    members=Form(None),
...
async def show_main_page(request: Request, session_token=Cookie(None)):
    with open_connect() as conn:
        ...
        context = {"status": status, "request": request}
